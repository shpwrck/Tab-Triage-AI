...
async function runForWindow(windowId) {
      ...
      afterTriage: async ({ rawGroups, tabs: triageCandidates, cap }) => {
        ...
        if (unsortedIds.length > 0) {
          ...
          try {
            ...
            groupedCount += unsortedIds.length;
          } catch (e) {
            console.warn("[auto-triage] applyAsTabGroup failed for Unsorted", e?.message ?? e);
          }
        }
        ...
        return { groupedCount, cachedGroups, cap };
      },
    }));
  } catch (e) {
  ...
  if (at.notify && result.groupedCount > 0) {
    const scopedCount = result.cap.applied
      ? `${result.groupedCount} of ${result.cap.originalCount}`
      : String(result.groupedCount);
    await notifyAutoTriage({
      title: "Tab Triage",
      message: `Grouped ${scopedCount} tabs into ${result.cachedGroups.length} clusters.`,
      priority: 0,
    });
  }
}
