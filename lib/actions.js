...
import { saveSessionWithResult } from "./storage.js";
...
export async function archiveGroup({ group, tabs }) {
  ...
  return { sessionId: session.id, closed: ids.length, saveResult };
}

export async function saveCloseRecoverySession({ title, groups, note }) {
  const normalizedGroups = (groups ?? [])
    .map((group, index) => normalizeRecoveryGroup(group, index, note))
    .filter(Boolean);
  const tabCount = normalizedGroups.reduce((sum, group) => sum + group.tabs.length, 0);
  ...
  const session = {
    ...
    kind: "recovery",
    createdAt: new Date().toISOString(),
    title: title || `Recovery: ${tabCount} closed tab${tabCount === 1 ? "" : "s"}`,
...
function normalizeRecoveryGroup(group, index, note) {
  const tabs = normalizeRecoveryTabs(group?.tabs);
  if (!tabs.length) return null;
  const label = String(group?.label || "").trim()
    || (tabs.length === 1 ? "Closed tab" : `Closed tabs ${index + 1}`);
  const summary = [
    ...normalizeRecoverySummary(group?.summary),
    note || `Saved before closing ${tabs.length} tab${tabs.length === 1 ? "" : "s"}.`,
    "Restore via Saved sessions to reopen.",
  ];
  return {
    label,
    emoji: group?.emoji || "",
...
function normalizeRecoveryTabs(tabs) {
  return (tabs ?? [])
    .map(tab => {
