...
// Reads paid status from ExtPay and writes verified results through to local
// settings. On refresh failure, preserves and returns the cached plan with an
// unverified status so UI can avoid falsely downgrading paid users.
export async function refreshPlan() {
  if (!BILLING_ENABLED) {
    await saveSettings({ plan: "free" });
    return { plan: "free", verified: true, source: "disabled" };
  }
  try {
    const user = await getExtPay().getUser();
    const plan = user?.paid ? "lifetime" : "free";
    await saveSettings({ plan });
    return { plan, verified: true, source: "extpay" };
  } catch (e) {
    const settings = await getSettings();
    const plan = settings.plan === "lifetime" ? "lifetime" : "free";
    console.warn("ExtPay getUser failed; using cached plan", e);
    return {
      plan,
      verified: false,
      source: "cache",
