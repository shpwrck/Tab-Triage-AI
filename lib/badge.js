...
// used in the last N hours, summed across all windows in the current
// profile. Driven by tab events; never calls an AI provider.
...
export async function updateBadge() {
  ...
  if (!hasStale && !hasDupes) {
    ...
    bg = BADGE_BG; fg = BADGE_FG;
    text = staleCount > 99 ? "99+" : String(staleCount);
    title = `Tab Triage AI — ${staleCount} stale tab${staleCount === 1 ? "" : "s"} (${label})`;
  } else {
